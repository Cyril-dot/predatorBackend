# Subscription Platform Backend (Spring Boot + Maven, Neon/PostgreSQL)

Pay-per-time-slot access platform:

## Database setup (Neon)

This app persists everything to a [Neon](https://neon.tech) Postgres database via Spring Data JPA / Hibernate.

1. Create a free Neon project at https://console.neon.tech.
2. Open your project → **Connection Details** → copy the **pooled** connection string.
3. Set these environment variables before running the app (see `.env.example`):

   ```bash
   export DATABASE_URL="jdbc:postgresql://<your-neon-host>/<your-db-name>?sslmode=require"
   export DATABASE_USERNAME="<your-neon-username>"
   export DATABASE_PASSWORD="<your-neon-password>"
   ```

4. Run the app normally (`mvn spring-boot:run` or the packaged jar). On first
   startup, Hibernate will automatically create all the required tables in
   your Neon database (`spring.jpa.hibernate.ddl-auto=update`, controlled by
   the `DB_DDL_AUTO` env var).
5. Once your schema is stable / you're running in production, set
   `DB_DDL_AUTO=validate` so Hibernate never auto-alters live tables — manage
   schema changes with migrations instead at that point.

No code changes are needed to switch databases later (e.g. moving off Neon to
another Postgres host) — just point `DATABASE_URL`/`DATABASE_USERNAME`/`DATABASE_PASSWORD`
at the new instance.
